...
public interface JobManagerMapper {
 ...
 /*   @Select("select " +
   ...
   /* @Select("select  his.job_id,job.name as job_name,job.description,his.start_time,his.end_time,his.execute_host,his.status,his.operator," +
    ...
    @Select("SELECT * FROM hera_action WHERE STATUS IS NULL AND LEFT(id, 12) <= DATE_FORMAT(NOW(),'%Y%m%d%H%i') AND LEFT(id, 8)>=DATE_FORMAT(NOW(),'%Y%m%d') AND auto='1' ORDER BY job_id,id ")
    //@Lang(HeraSelectLangDriver.class)
    List<HeraActionAll> findNoRunJobHistoryByStatus();
